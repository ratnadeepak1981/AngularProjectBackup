const StudentDashboardModule = (function () {
    ...
    async function loadProfile() {
        try {
            ...
            const studentName = student.name || student.Name || student.fullName || student.FullName || (AuthService.getUserProfile()?.fullName) || '';
            document.getElementById('student-index-badge').textContent = `Index: ${student.indexNumber || student.IndexNumber || 'N/A'}`;
            document.getElementById('profile-name').value = studentName;
            document.getElementById('profile-index').value = student.indexNumber || student.IndexNumber || '';
            document.getElementById('profile-faculty').value = student.facultyName || student.FacultyName || student.faculty?.name || student.Faculty?.Name || 'Faculty of Applied Sciences';
