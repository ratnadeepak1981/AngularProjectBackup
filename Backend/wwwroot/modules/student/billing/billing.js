            ...
            tbody.innerHTML = loadedLedgerItems.map(item => {
                ...
                return `
                    <tr>
                        ...
                        <td><span class="badge badge-${isPaid ? 'approved' : 'held'}">${item.status}</span></td>
                        <td>${isPaid ? `Receipt #${item.receiptNumber || 'REC-101'}<br><small>${new Date(item.paidAt || item.updatedAt || new Date()).toLocaleDateString()}</small>` : '-'}</td>
                        <td>
                            ${!isPaid 
